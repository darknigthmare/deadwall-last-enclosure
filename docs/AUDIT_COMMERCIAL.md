...
Un verrou supplémentaire interdit de tracer un mur avancé si la cité a perdu le palier requis après sélection de l'outil. L'amélioration d'un mur exige désormais un chantier terminé. Le comptage des ouvriers affectant l'entretien des corps est effectué une seule fois par mise à jour des bâtiments.
...
Le coût affiché du fusilier inclut les 10 ferrailles réellement débitées. Une doctrine verrouillée annonce son prochain palier au lieu de prétendre que toutes les recherches sont terminées. La jauge d'alerte reconnaît les 15 secondes de reconnaissance et reste bornée entre 0 et 100 %. L'objectif d'électrification exige effectivement un générateur terminé et du carburant. L'objectif des douze segments annonce la préparation d'une enceinte, pas une fermeture topologique qui n'est pas automatiquement vérifiée.
...
Les portes continuent à laisser passer les alliés et à résister aux infectés. Elles n'ont pas encore de commande manuelle ouvrir/fermer : le champ `gateMode` historique n'est pas présenté comme un système de portes pilotables. La nouvelle navigation ne fait pas traverser les murs et ne modifie pas le champ de flux des hordes.

Les six doctrines sont achetables une seule fois et produisent des effets mesurés sur collecte/construction, résistance des murs, dégâts des défenses, corps, énergie et annonce des vagues. Les trois difficultés modifient réellement la réserve initiale, la collecte, le temps calme, le nombre, la santé et les dégâts des infectés. Les scénarios automatisés valident ces règles, pas un équilibre de longue durée avec un panel de joueurs. La recherche reste instantanée, séquentielle et sans branches alternatives ; aucune campagne scénarisée ou progression entre parties n'est annoncée comme livrée. Les brigades sanitaires réduisent la pression des corps ; aucun bonus distinct de soins n'est vérifié.
