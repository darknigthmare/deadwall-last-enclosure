...
for(const variant of variants){
  ...
  try{
    ...
    await page.waitForFunction(async files=>{const cache=await caches.open('deadwall-v1.0.0-r9');return(await Promise.all(files.map(file=>cache.match(new URL(file,location.href).href)))).every(Boolean);},required);check('cache r9 contient récit, sauvegarde, commandement et atlas');
