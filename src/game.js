(function initDeadwall() {
  ...
  class Game {
    ...
    requestNewGame() {
      if(this.state!=='menu'||this.activeOverlay!==this.ui.mainMenu)return false;
    ...
    getSquadSummary() {
      ...
      return state.groups.map((group,index)=>{
        const units=this.units.filter(u=>u.kind==='soldier'&&!u.dead&&u.health>0&&u.squad===index),retreat=group.order==='retreat';
        return{index,label:C.SQUAD_RULES.labels[index],order:group.order,rally:{...group.rally},count:units.length,selected:state.selected===index,
          blocked:units.filter(u=>{const target=retreat?core:this.squadFormationTarget(u,group);return !target||(u.navigation?.cells===null&&dist(u,target)>(retreat?C.SQUAD_RULES.retreatRadius:C.SQUAD_RULES.rallyRadius));}).length};
      });
    }
    ...
    updateInteraction(dt) {
      ...
      for (const b of this.world.buildings.values()) {
        ...
        if (carried > .01 && b.completed && (b.type === 'core' || b.type === 'warehouse') && d < storageD && this.workerCanWorkAt(p, b, 100)) { storageD = d; storage = b; }
      }
      ...
      let action = null;
      if (storage && depositable > 0 && dist(storage,p) < 100) {
    ...
    melee() {
      ...
      for (const z of this.nearbyZombies(p.x, p.y, 55)) {
        ...
        z.health -= 36; z.stagger = .35; hit = true;
    ...
    updateProjectiles(dt) {
      ...
      for (const p of this.projectiles) {
        ...
        let target = null, impact = Infinity;
        // Swept circle collision removes frame-rate tunnelling; shots still pass over walls.
        for (const z of this.nearbyZombies(p.x + dx / 2, p.y + dy / 2, length / 2 + maxRadius + p.radius + 3)) {
          ...
          if (time < impact || (time === impact && z.id < target.id)) { target = z; impact = time; }
        }
        const travel = target ? impact : 1;
